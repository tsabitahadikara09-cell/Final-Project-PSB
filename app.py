...
with main_tabs[0]:
    ...
    with sub[0]:
        st.plotly_chart(plot_two_signals(d["n"], d["heel"], d["toe"], "Heel / FSR Biru", "Toe / FSR Merah", "INPUT"), use_container_width=True)
        st.plotly_chart(plot_two_signals(d["n"], d["heel_filt"], d["toe_filt"], "Heel Filtering", "Toe Filtering", "OUTPUT / HASIL FILTERING"), use_container_width=True)
        fig_seg = go.Figure()
        ...
        st.plotly_chart(base_layout(fig_seg, "HEEL dan TOE PHASE DETECTION", "Waktu (s)", "Amplitudo"), use_container_width=True)
        j_in, j_out = plot_joint_input_output(d)
        st.plotly_chart(j_in, use_container_width=True)
        st.plotly_chart(j_out, use_container_width=True)
    with sub[1]:
    ...
    with sub[3]:
        ...
        for tab, name, raw, filt, color in zip(single, ["Hip", "Knee", "Ankle"], [d["hip"], d["knee"], d["ankle"]], [d["hip_filt"], d["knee_filt"], d["ankle_filt"]], ["red", "green", "blue"]):
            with tab:
                st.plotly_chart(base_layout(go.Figure(go.Scatter(x=d["n"], y=raw, mode="lines", name=f"{name} Input", line=dict(color=color))), f"{name.upper()} JOINT INPUT", "n (sample)", "Degree"), use_container_width=True)
                st.plotly_chart(base_layout(go.Figure(go.Scatter(x=d["n"], y=filt, mode="lines", name=f"{name} Filtering", line=dict(color=color))), f"{name.upper()} JOINT OUTPUT / HASIL FILTERING", "n (sample)", "Degree"), use_container_width=True)
...
with main_tabs[5]:
    ...
    st.plotly_chart(plot_stft(d, stft_choice), use_container_width=True)
